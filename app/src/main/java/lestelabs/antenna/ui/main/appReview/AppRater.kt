...
object AppRater {
    ...
    private const val SECS_UNTIL_PROMPT = 0 //Min number of days
    private const val LAUNCHES_UNTIL_PROMPT = 0 //Min number of launches

    private val tabName = "AppRater"
    private var crashlyticsKeyAnt = ""

    fun app_launched(activity:Context, mContext: Context) {
        ...
        val prefs = activity.getSharedPreferences("sharedpreferences", Context.MODE_PRIVATE)
        Log.d("cfauli","Apprater started pleasecontribute " + prefs.getBoolean("please_contribute", false))
        Log.d("cfauli","Apprater started dontshowagain " + prefs.getBoolean("dontshowagain", false))
        if (prefs.getBoolean("dontshowagain", false) || prefs.getBoolean("voteCasted", false) ) {
            return
        }
        ...
        Log.d("cfauli","Apprater launch_count " + launch_count)

        // Get date of first launch
        var date_firstLaunch = prefs.getLong("date_firstlaunch", 0)
        Log.d("cfauli","Apprater date_firstLaunch " + date_firstLaunch)
        ...
        if (launch_count >= LAUNCHES_UNTIL_PROMPT) {
            if (System.currentTimeMillis() >= date_firstLaunch +
                ...
                Log.d("cfauli","Apprater show Dialog")
                // Control point for Crashlitycs
                crashlyticsKeyAnt = Crashlytics.controlPointCrashlytics(tabName, Thread.currentThread().stackTrace, crashlyticsKeyAnt)
    ...
    fun showRateDialog(mContext: Context, editor: SharedPreferences.Editor?) {
        ...
        builder.setPositiveButton("OK") { dialog, which ->
            ...
            if (editor != null) {
                editor.putBoolean("voteCasted", true)
                editor.commit()
            }
        }
